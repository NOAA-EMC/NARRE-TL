     ...
     +              nv,ifunit,jf,iens,Lp,Lt,jpd10
        ...
        do 200 k=1,iens
          ...
          jpd12=dProbLevel(nv,1)
           ...
           call readGB2(ifunit(k),jpdtn,6,1,200,0,jp27,gfld,iret) !Total cloud
            if(iret.eq.0) then 
             totcld(:,k)=gfld%fld
            else
             !write(*,*) 'Total clodu missing in file',ifunit(k)
             !write(*,*) 'In RAP try ...'
             call readGB2(ifunit(k),jpdtn,6,1,10,0,jp27,gfld,iret)
             totcld(:,k)=gfld%fld
            end if
 
           call readGB2(ifunit(k),jpdtn,3,5,2,0,jp27,gfld,iret)   !Cloud base
            if(iret.eq.0) then
             cldbas(:,k)=gfld%fld
            else
